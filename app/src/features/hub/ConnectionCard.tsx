...
export function ConnectionCard({
      ...
      : status === 'error'
        ? [
            ...
            profile.lastConnectedAt
              ? `last ok ${formatRelative(new Date(profile.lastConnectedAt).getTime())}`
              : 'never connected',
          ]
        : [
            `${savedCount} saved`,
            profile.lastConnectedAt
              ? `last ${formatRelative(new Date(profile.lastConnectedAt).getTime())}`
              : 'never connected',
          ];
  ...
  return (
    <Pressable
      onPress={onOpen}
      style={[styles.card, kind === 'fault' && styles.cardFault]}
    >
      <View style={styles.top}>
        ...
        {snapshot?.counters.rx && status === 'connected' ? (
          <Text style={styles.badge}>{snapshot.counters.rx}</Text>
        ) : null}
