...
export function HubScreen({ navigation }: Props) {
  ...
  const liveCount = profiles.filter(
    p => snapshots[p.id]?.status === 'connected',
  ).length;
  const subCount = profiles.reduce(
    (acc, p) => acc + (snapshots[p.id]?.subscriptions.length ?? 0),
    0,
  );
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ConnectionCard
            ...
            onOpen={() =>
              navigation.navigate('Workspace', { profileId: item.id })
            }
            onToggleConnect={() =>
              snapshots[item.id]?.status === 'connected'
                ? disconnect(item.id)
                : connect(item.id)
            }
            onEdit={() =>
        ...
        ListFooterComponent={
          <>
            ...
            <Text style={styles.footerNote}>
              Stored on device · encrypted at rest{'\n'}no cloud sync
            </Text>
          </>
        }
      />
...
const styles = StyleSheet.create({
  ...
  gear: { fontSize: 26, color: colors.textSecondary },
  summary: {
