...
export function PublishTab({
  ...
  const [jsonEditor, setJsonEditor] = useState<JsonEditor>('text');
  ...
  function switchToKeyValue() {
    const parsed = jsonTextToRows(payload);
    if (parsed === null) {
      show('key-value view needs a flat JSON object — fix the JSON first');
      return;
    }
    setRows(parsed.length > 0 ? parsed : [emptyRow()]);
    setJsonEditor('keyvalue');
  }

  function updateRows(next: KVRow[]) {
    setRows(next);
    setPayload(rowsToJsonText(next));
  }
  ...
  function pin() {
    ...
    addSnippet({
      profileId,
      name: topic.trim().split('/').pop() || topic.trim(),
      topic: topic.trim(),
      payload,
      qos,
      retain,
