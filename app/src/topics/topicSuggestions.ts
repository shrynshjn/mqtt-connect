...
const WEIGHTS: Record<TopicUsageKind, number> = { subscribe: 1.0, publish: 1.0, observed: 0.4 };
...
function record(k: string, cap: number, topic: string, kind: TopicUsageKind, now: number): void {
  const entries = read(k);
  const idx = entries.findIndex(e => e.topic === topic);
  const weight = WEIGHTS[kind];
  if (idx === -1) {
    entries.push({ topic, kinds: [kind], count: 1, lastUsedAt: now, score: weight });
  } else {
    const e = entries[idx];
...
export function recordTopicUsage(profileId: ProfileId, topic: string, kind: TopicUsageKind): void {
  const now = Date.now();
...
export function removeTopicUsage(profileId: ProfileId, topic: string): void {
  write(key(profileId), read(key(profileId)).filter(e => e.topic !== topic));
}
...
export function suggestTopics(profileId: ProfileId, query: string, limit = 6): string[] {
  const now = Date.now();
  const q = query.trim().toLowerCase();
  const local = read(key(profileId)).map(e => ({ topic: e.topic, score: decayedScore(e, now) }));
  const global = read(GLOBAL_KEY).map(e => ({ topic: e.topic, score: decayedScore(e, now) * 0.3 }));
